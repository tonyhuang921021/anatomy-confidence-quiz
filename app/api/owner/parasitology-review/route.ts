...
type ParasitologyReviewRequestBody = {
  accessToken?: string;
};
...
export async function POST(request: NextRequest) {
  ...
  const body = (await request.json().catch(() => null)) as ParasitologyReviewRequestBody | null;
  ...
  const htmlPath = path.join(
    ...
    "data",
    "owner",
    "parasitology_board_review_interactive_v2.html"
  );
