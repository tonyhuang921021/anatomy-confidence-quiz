...
15. 題號格式請優先使用「西元年-第幾次-卷碼-Q題號」，例如 2022-1-1301-Q025。卷碼常見如 1301、2301、5301；如果來源沒有卷碼，才用 2022-1-Q025。不要自己推測或編造 MOEX ID。
16. 只有當公開來源或我貼給你的內容已經明確出現完整 MOEX ID 時，才可以輸出 MOEX-110020-1301-Q026 這種格式；不要把年份、次別、卷碼自行組成 MOEX ID，因為很容易錯。
17. 如果查不到卷碼，不要放棄，也不要編卷碼；可以輸出 2019-2-Q008 這種簡寫。請注意這裡的 2 是「第 2 次考試」，不是「醫學二」。網站會再用 note-meta 的 subject 輔助對應本地題庫。
18. 如果你已經實際上網搜尋，但真的找不到高度相關的正式考古題，questionLinks 可以留空，並在 summary 後補一句「未找到可確認題號」；但不要自己編題號。
19. 如果正文原本有明確題號，而且你確定是網站題庫的題目，可以額外用這個短碼獨立放一行，但這不是必要：
[question-note id="題目ID" title="簡短題目標題"]
20. 不要加「以下是整理後內容」這種開場白。${formatQuestionCandidates(candidates)}
