...
請遵守：
1. 最上方先輸出一個 note-meta 區塊，讓網站自動分類。格式固定如下：
\`\`\`note-meta
title: 這篇筆記的清楚標題
...
7. 不要輸出 HTML，不要輸出圖片連結，不要包成 JSON。
8. 如果原文有題號，而且你確定是網站題庫的題目，可以額外用這個短碼獨立放一行：
[question-note id="題目ID" title="簡短題目標題"]
9. 如果沒有明確題號，不要硬產生 question-note。
10. 不要加「以下是整理後內容」這種開場白。
