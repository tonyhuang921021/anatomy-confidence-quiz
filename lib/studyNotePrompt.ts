...
13. 題號格式請優先使用網站可辨識的 ID，例如 MOEX-110020-1301-Q018。如果你在公開來源只查到年份、次別和題號，請用「2014-2-Q017」這種清楚格式，並盡量避免含糊描述。
14. 如果你已經實際上網搜尋，但真的找不到高度相關的正式考古題，questionLinks 可以留空，並在 summary 後補一句「未找到可確認題號」；但不要自己編題號。
15. 如果正文原本有明確題號，而且你確定是網站題庫的題目，可以額外用這個短碼獨立放一行，但這不是必要：
[question-note id="題目ID" title="簡短題目標題"]
16. 不要加「以下是整理後內容」這種開場白。${formatQuestionCandidates(candidates)}
