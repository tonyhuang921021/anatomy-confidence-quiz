...
summary: 50 字內摘要
tags: tag1, tag2, tag3
searchKeywords: 代表性搜尋詞1, 代表性搜尋詞2, 英文關鍵詞
...
14. searchKeywords 請放 2-6 個最能代表這篇筆記的「題庫搜尋詞」，不要寫成完整句子。目標是讓網站用這些詞搜尋本地題庫並自動拉進相關題目。例如問咽弓發育，可以放：咽弓, 咽囊, 咽裂, pharyngeal arch。中英文都可以，但請只放真的可能出現在題幹、章節、選項或詳解裡的詞。
15. searchKeywords 可以和 tags 不同；tags 是分類用，searchKeywords 是找題目用，請優先挑能命中題目的詞。
16. 查證題號時使用的來源、搜尋過程、URL、引用依據只用來幫你判斷題號，不要寫進 Markdown 正文，也不要新增「題號查證來源」「相關考古題來源」「搜尋紀錄」這類段落。題號放在最上方 note-meta 的 questionLinks 就好。
17. 題號格式請優先使用「西元年-第幾次-卷碼-Q題號」，例如 2022-1-1301-Q025。卷碼常見如 1301、2301、5301；如果來源沒有卷碼，才用 2022-1-Q025。不要自己推測或編造 MOEX ID。
18. 只有當公開來源或我貼給你的內容已經明確出現完整 MOEX ID 時，才可以輸出 MOEX-110020-1301-Q026 這種格式；不要把年份、次別、卷碼自行組成 MOEX ID，因為很容易錯。
19. 如果查不到卷碼，不要放棄，也不要編卷碼；可以輸出 2019-2-Q008 這種簡寫。請注意這裡的 2 是「第 2 次考試」，不是「醫學二」。網站會再用 note-meta 的 subject 輔助對應本地題庫。
20. 如果你已經實際上網搜尋，但真的找不到高度相關的正式考古題，questionLinks 可以留空，並在 summary 後補一句「未找到可確認題號」；但不要自己編題號。這時仍要填 searchKeywords，讓網站可以用本地題庫搜尋補題。
21. 如果正文原本有明確題號，而且你確定是網站題庫的題目，可以額外用這個短碼獨立放一行，但這不是必要：
[question-note id="題目ID" title="簡短題目標題"]
22. 不要加「以下是整理後內容」這種開場白。${formatQuestionCandidates(candidates)}
...
你不需要重寫筆記正文，也不要輸出題幹、選項、答案、詳解或來源查證段落。只需要輸出一段可以貼回網站的題號清單和搜尋詞。

請盡量上網查公開來源，找和筆記核心觀念高度相關的正式考古題。不要憑印象亂編題號；找不到足夠題目時，少於 ${count} 題也可以。
如果你不能上網或找不到可確認題號，請直接回 questionLinks:，但仍然要給 searchKeywords，讓網站用本地題庫自動搜尋。不要放棄、不要編題號，也不要輸出很長的解釋。
...
- 如果公開來源真的沒有卷碼，才可以暫時用 2019-2-Q008；但請不要自己猜卷碼，也不要因為缺卷碼就整段放棄。這裡的 2 是「第 2 次考試」，不是「醫學二」。
...
questionLinks: 2022-1-1301-Q025, 2020-2-1301-Q021, 2011-1-1301-Q017
searchKeywords: 咽弓, 咽囊, 咽裂, pharyngeal arch

如果找不到可確認題號，請回：
questionLinks:
searchKeywords: 代表性搜尋詞1, 代表性搜尋詞2
